Attribute VB_Name = "cumulativeList4"
...
'последняя версия'
'работает для объекта Реконструкция здания. Отделение медицинской реабилитации
...
Set coefTransp = Seach("перевозка", seachRange, 2)
...
For j = 2 To totalByPosition.Count
    ...
    For i = beginning To totalByPosition(j)
        If Cells(i, 3).Value Like "Перевозка *" Or Cells(i, 3).Value Like "Погрузка *" Then
            ...
            Cells(i, 14).Formula = "=round(M" & i & "*L" & i & ",2)"
        Else
            Call filCurrentPrices(i)
        End If
    Next
    Cells(totalByPosition(j), 14).Formula = "= SUM(N" & beginning + 1 & ":N" & totalByPosition(j) - 1 & ")"
    'If Cells(totalByPosition(j), 14).MergeCells = True Then
    '    Call cancelMerge("M", totalByPosition(j), "N", totalByPosition(j), 1)
    'End If
...
If ans = 1 Then
    'Cells(totalByEstimate(1), 14).Formula = "= N" & totalByEstimate(1)
